...
open(SANDER_EQ_AA, ">"."$protein_label"."_randdeploy.in") or die "could not open SANDER_EQ file\n";
	print SANDER_EQ_AA "Equilibration\n"; 
	print SANDER_EQ_AA "&cntrl\n"; 
	print SANDER_EQ_AA "imin=0,\n"; 		
	print SANDER_EQ_AA "ntx=1,\n"; 		
...
######################################################################
for (my $jj = 0; $jj < $num_runs; $jj++) {
print "\ starting MD run for "."rand_$protein_label"."_deploy.out\n";
system("export CUDA_VISIBLE_DEVICES=0\n"."$run_method -O -i $protein_label"."_randdeploy.in -o rand_$protein_label"."_deploy.out -p $prmtop"."_$protein_label.prmtop -c eq_$protein_label.rst -r rand_$protein_label"."_deploy.rst -x rand_$protein_label"."_deploy.nc -inf rand_$protein_label"."_deploy.info &");
...
chomp $pid;
	...
	#print "$chkpid\n";
       	chomp $chkpid;
        #print "check pmemd PID = "."$pid\t"."$chkpid\n"; 
        if ($pid eq $chkpid) {print "\ $seconds"." secs into MD run\t"."rand_$protein_label"."_deploy.out\n";}
	#	if ($pid ne $chkpid) {$pid = ''; $chkpid = '';}
		    #system("tail -f amino_acid_min_$protein_label.out".".out")          
	}
print "\ end MD run $jj\n";
...
######################################################################
for (my $jj = 0; $jj < $num_runs; $jj++) {
print "\ starting MD run for "."prod_$protein_label"."_deploy.out\n";
system("export CUDA_VISIBLE_DEVICES=0\n"."$run_method -O -i $protein_label"."_prod.in -o prod_$protein_label"."_deploy.out -p $prmtop"."_$protein_label.prmtop -c rand_$protein_label"."_deploy.rst -r prod_$protein_label.rst -x prod_$protein_label"."_deploy.nc -inf prod_$protein_label"."_deploy.info &");
...
chomp $pid;
	...
	#print "$chkpid\n";
       	chomp $chkpid;
        #print "check pmemd PID = "."$pid\t"."$chkpid\n"; 
        if ($pid eq $chkpid) {print "\ $seconds"." secs into MD run\t"."prod_$protein_label"."_deploy.out\n";}
	#	if ($pid ne $chkpid) {$pid = ''; $chkpid = '';}
		    #system("tail -f amino_acid_min_$protein_label.out".".out")          
	}
print "\ end MD run for learner deploy\n";
