...
model = "1"
pdb_file = "%s.pdb" % (rID)
attr_file = "testingData_%s_1/indAAdrmsf_attr/drmsfATTR_%s_1.dat" % (rID, qID)
...
mid_color = ""
mid_value = ""
if attr == "delta" or attr == "deltaKL" or attr == "dRMSF":
 mid_color = "white"
 mid_value = 0;
...
for x in range(1,frame_groups):
    attr_file = "testingData_%s_1/indAAdrmsf_attr/drmsfATTR_%s_%s.dat" % (rID, qID, x)
    rc("defattr %s raiseTool false" % (attr_file))
    col_string = "%s %s %s %s %s %s novalue tan" % (max_value, max_color, mid_value, mid_color, min_value, min_color)
    rc("colorkey  .08,.1  .10,.800  %s" % (col_string))
    rc("rangecolor %s,%s %s #%s" % (attr, rep_key, col_string, model)) 
