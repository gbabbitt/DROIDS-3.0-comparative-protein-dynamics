...
varfile = open("variant_list.txt", "r")
varfile_lines = varfile.readlines()
for xx in range(1, len(varfile_lines)):
    filename = varfile_lines[xx]
    ...
    # append reporters
    print ('MD production run for', 'prod_'+PDBid+'_deploy.nc')
    simulation.reporters.append(pmd.openmm.NetCDFReporter('prod_'+PDBid+'_deploy.nc', 200))
    ...
    print('prod_'+PDBid+'_deploy.nc is done')
print("VARIANT LIST MD RUNS ARE DONE")