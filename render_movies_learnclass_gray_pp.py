...
frame_number = int(framenumberline[1])
print frame_number
framestepline=lines_list[1].split()
print framestepline
frame_step = int(framestepline[1])
print frame_step
print lines_list
framegroupline=lines_list[2].split()
print framegroupline
frame_groups = int(framegroupline[1])
print frame_groups
...
assert(frame_groups > 0)
assert(frame_step > 0)

model = "1"
pdb_file = "%s.pdb" % (rID)
attr_file = "testingData_%s_1/indAAclass_attr/classATTR_%s_1.dat" % (rID, qID)
...
if rep == "ribbonsurface":
 rc("ribbon #%s" % (model))
 rc("surface #%s" % (model))
 rc("surftransparency 60 #%s" % (model))
 rep_key = "r,s"
...
attr_file = "testingData_%s/indAAclass_attr/classATTR_%s_1.dat" % (rID, qID)
rc("movie record")
rc("coordset #%s 1, holdSteady #%s:%s@CA load true" %(model, model, central_residue))
rc("scale 1.02 100")
x = 1
for x in range(1,frame_groups):
    attr_file = "testingData_%s_1/indAAclass_attr/classATTR_%s_%s.dat" % (rID, qID, x)
    rc("defattr %s raiseTool false" % (attr_file))
    col_string = "%s %s %s %s %s %s novalue gray50" % (max_value, max_color, mid_value, mid_color, min_value, min_color)
    rc("colorkey  .08,.1  .10,.800  %s" % (col_string))
    rc("rangecolor %s,%s %s #%s" % (attr, rep_key, col_string, model)) 
    rc("rangecolor %s,a %s #%s" % (attr, col_string, model))
    rc("wait %s" % (frame_step))
rc("movie stop")
rc("movie status")
