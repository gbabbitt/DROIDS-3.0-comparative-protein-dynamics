...
$mw -> title("DROIDS - Learned Representation in UCSF Chimera"); # Titles the main window
$mw->setPalette("gray");
...
my $seqFrame = $mw->Frame(	-label => "PROTEIN COLOR SCHEME",
     ...
     my $col1Radio = $seqFrame->Radiobutton(-text=>"'stoplight' color scheme = red/green for relative dFLUX, blue for classifier",
						-foreground => 'darkgreen',
                              -value=>"c1",
						-variable=>\$colorScheme
                              );
	my $col2Radio = $seqFrame->Radiobutton(-text=>"'temperature' color scheme = blue/red for relative dFLUX, green for classifier",
						-foreground => 'darkgreen',
                              -value=>"c2",
						-variable=>\$colorScheme
                              );
...
print "Enter number of frames/step in time series (default = 10)\n";
my $stepsize = <STDIN>;
chop($stepsize);
if ($stepsize eq ''){$stepsize = 10;}
